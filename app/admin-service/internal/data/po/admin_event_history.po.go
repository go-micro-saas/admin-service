...
type AdminEventHistory struct {
	...
	EventStatus       enumv1.UserEventStatusEnum_UserEventStatus `gorm:"column:event_status" json:"event_status"`               // 事件状态；1：成功，2：失败，3：重试中，4：重试成功，5：重试失败。。。
...
func DefaultAdminEventHistory(eventName, eventContent string) *AdminEventHistory {
	var (
		now          = time.Now()
		historyModel = &AdminEventHistory{
			Id:                0,
			CreatedTime:       now,
			UpdatedTime:       now,
			EventName:         eventName,
			EventStatus:       enumv1.UserEventStatusEnum_SUCCESS,
